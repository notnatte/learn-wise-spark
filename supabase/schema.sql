...
-- Lessons policies
CREATE POLICY "Lessons in published courses are viewable by everyone" 
...
CREATE POLICY "Users can view their own streaks" 
  ON public.learning_streaks FOR SELECT USING (user_id = auth.uid());

CREATE POLICY "Users can update their own streaks" 
  ON public.learning_streaks FOR INSERT WITH CHECK (user_id = auth.uid());

CREATE POLICY "Users can update their own streaks" 
  ON public.learning_streaks FOR UPDATE USING (user_id = auth.uid());
